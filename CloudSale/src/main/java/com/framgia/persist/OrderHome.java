...
public class OrderHome {
	...
	public Order findById(java.lang.Integer id) {
		...
		try {
			Order instance = (Order) sessionFactory.getCurrentSession().get("com.framgia.model.Order", id);
			if (instance == null) {
	...
	public List<Order> findByExample(Order instance) {
		...
		try {
			List<Order> results = (List<Order>) sessionFactory.getCurrentSession()
					.createCriteria("com.framgia.model.Order").add(create(instance)).list();
