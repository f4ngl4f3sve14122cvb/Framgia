...
public class OrderDetailHome {
	...
	public OrderDetail findById(com.framgia.model.OrderDetailId id) {
		log.debug("getting OrderDetail instance with id: " + id);
		try {
			OrderDetail instance = (OrderDetail) sessionFactory.getCurrentSession().get("com.framgia.model.OrderDetail",
					id);
			if (instance == null) {
	...
	public List<OrderDetail> findByExample(OrderDetail instance) {
		...
		try {
			List<OrderDetail> results = (List<OrderDetail>) sessionFactory.getCurrentSession()
					.createCriteria("com.framgia.model.OrderDetail").add(create(instance)).list();
